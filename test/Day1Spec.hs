...
import Day1 (findFirstEntry, findThreeEntries)
...
test_findFirstEntry =
  testCase "findFirstEntry with empty list returns Nothing" $
    findFirstEntry 2020 [] @?= Nothing

test_findFirstEntry_overlappingList =
  testCase "findFirstEntry with list of overlapping 2020 returns Nothing" $
    findFirstEntry 2020 [2045, 2040, 3030] @?= Nothing

test_findFirstEntry_singleValue =
  testCase "findFirstEntry with single value different from 2020 list returns Nothing" $
    findFirstEntry 2020 [2045] @?= Nothing

test_findFirstEntry_mixedNotMatching2020 =
  testCase "findFirstEntry with mixed values not matching 2020 list returns Nothing" $
    findFirstEntry 2020 [1020, 1010, 1, 3, 4, 2045] @?= Nothing

test_findFirstEntry_mixedMatching2020 =
  testCase "findFirstEntry with provided input dataset returns the expected number (909)" $
    findFirstEntry 2020 inputData @?= Just 909

test_findThreeEntries_mixedMatching2020 =
  testCase "findThreeEntries with provided input dataset returns the expected number (74, 382)" $
    findThreeEntries 2020 inputData @?= Just (74, 382)
