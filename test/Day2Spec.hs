...
test_parseInvalidLine =
  testCase "parseLine with invalid line returns Nothing" $
    parseLine (pack "kkk") @?= Nothing

test_parseValidLine =
  testCase "parseLine with valid line returns the expected password" $
    parseLine (pack "8-9 n: rnsnvlrqdq") @?= Just (Password {char = 'n', first = 8, second = 9, password = "rnsnvlrqdq"})

test_parseEntry =
  testCase "parsePassword valid" $
    parsePassword ["8", "9", "n", "rnsnvlrqdq"] @?= Just (Password {char = 'n', first = 8, second = 9, password = "rnsnvlrqdq"})

test_isPasswordValid_nothing =
  testCase "isPasswordValid with nothing as password" $
    isPasswordValid Nothing @?= False

test_isPasswordValid_invalid =
  testCase "isPasswordValid with invalid password" $
    isPasswordValid (Just password) @?= False
  where
    password =
      Password
        { char = 'n',
...
test_isPasswordValid_valid =
  testCase "isPasswordValid with valid password" $
    isPasswordValid (Just password) @?= True
  where
    password =
      Password
        { char = 'n',
...
test_countValidPasswords =
  testCase "countValidPasswords" $
    countValidPasswords smallPortionOfData @?= 2
  where
    smallPortionOfData =
      [ pack "3-4 j: tjjj",
        pack "7-10 h: nhhhhhgghphhh",
        pack "7-13 j: tpscbbstbdjsjbtcpj"
      ]

test_countValidPasswords_input =
  testCase "countValidPasswords with real data input" $
    countValidPasswords input @?= 483

test_isNewPasswordValid_nothing =
  testCase "isPasswordValidWithNewPolicy with nothing as password" $
    isPasswordValidWithNewPolicy Nothing @?= False

test_isNewPasswordValid_valid =
  testCase "isPasswordValidWithNewPolicy with valid password (abcde)" $
    isPasswordValidWithNewPolicy (Just password) @?= True
  where
    password =
      Password
        { char = 'a',
...
test_isNewPasswordValid_invalid_abcde =
  testCase "isPasswordValidWithNewPolicy with invalid password (abcde)" $
    isPasswordValidWithNewPolicy (Just password) @?= False
  where
    password =
      Password
        { char = 'b',
...
test_isNewPasswordValid_invalid_ccccccccc =
  testCase "isPasswordValidWithNewPolicy with invalid password (ccccccccc)" $
    isPasswordValidWithNewPolicy (Just password) @?= False
  where
    password =
      Password
...
test_countValidNewPasswords =
  testCase "countValidPasswordsWithNewPolicy" $
    countValidPasswordsWithNewPolicy smallPortionOfData @?= 1
  where
    smallPortionOfData =
      [ pack "3-4 j: tjjj",
        pack "7-10 h: nhhhhhgghphhh",
        pack "7-13 j: tpscbbjtbdjshbtcpj"
      ]

test_countValidNewPasswords_input =
  testCase "countValidPasswordsWithNewPolicy with real data input" $
    countValidPasswordsWithNewPolicy input @?= 482
